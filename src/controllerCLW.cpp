...
private:
		...
		ros::Subscriber avoidBigJumpvel_sub;
		...
		bool landcmd;
		//Velocity Components
		crazyflie_driver::Hover avoid, field, avoidBigJump, levy;
...
public:
	...
	cfCtrl(){
    	...
    	avoidBigJumpvel_sub = nh.subscribe("avoidBigJumpsVel", 10, &cfCtrl::getAvoidBigJumpVel, this);
	...
	void getAvoidBigJumpVel(const geometry_msgs::Twist& msg){
		crazyflie_driver::Hover vel;
		vel.header.stamp = ros::Time::now();
		vel.vx=xvel; vel.vy=0.0; vel.zDistance=height;
		//cout<<msg.angular.z<<endl;
