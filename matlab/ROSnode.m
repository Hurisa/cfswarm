...
axis([-2.05,2.05,-1.55,1.55]);
    for pp=1:size(x,2)
        ...
        scatter(x(:,pp),y(:,pp),sz;,[255,140,0]./255,'filled');
...
%pose5=receive(sub5,10);
%pose4=receive(sub4,10);
sr=0.1;
%p=scatter(points(:,1),points(:,2),2,[26,255,0]./255,'filled');
   pfPlot=scatter(pf.X,pf.Y,sz,[0,255,255]./255,'filled');
 %  plPlot=scatter(pl.X,pl.Y,5,[26,255,0]./255,'filled');
while(1)
    
   h1=scatter(pose1.Position.X,pose1.Position.Y,20,'MarkerEdgeColor',[217, 68, 150]./255,'MarkerFaceColor',[217, 68, 150]./255);
   f1=rectangle('Position',[pose1.Position.X-sr/2,pose1.Position.Y-sr/2,sr,sr],'EdgeColor',[217, 68, 150]./255);
...
%    h5=scatter(pose5.Position.X,pose5.Position.Y,20,'MarkerEdgeColor',[217, 68, 150]./255,'MarkerFaceColor',[217, 68, 150]./255);
%    f5=rectangle('Position',[pose5.Position.X-sr/2,pose5.Position.Y-sr/2,sr,sr],'EdgeColor',[217, 68, 150]./255);
   


   pose1=receive(sub1,10);pose2=receive(sub2,10);%pose3=receive(sub3,10);pose4=receive(sub4,10);%pose5=receive(sub5,10);
